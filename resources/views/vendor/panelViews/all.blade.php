...
<br><br>
@if ($current_entity == 'Trace')
	<a href="{!! url('panel/'.$current_entity.'/export?isTable3Item='.$isTable3Item."&isTable4Item=".$isTable4Item) !!}" class="btn btn-info">{!! \Lang::get('panel::fields.exportAsExcel') !!}</a>
@elseif ($current_entity == 'PingContent')
	<a href="{!! url('panel/'.$current_entity.'/export?trace_id='.$trace_id) !!}" class="btn btn-info">{!! \Lang::get('panel::fields.exportAsExcel') !!}</a>
@endif
